 ...
 * 자동 이벤트 수집 기능 포함 (중복 초기화 방지)
 */

// 전역 세션 활동 업데이트 함수 (무한 재귀 방지)
if (!window.updateSessionActivity) {
    window.updateSessionActivity = function() {
        try {
            ...
            // 유저 속성 추적기 업데이트 (있는 경우)
            if (window.userTracker && typeof window.userTracker.updateSessionActivity === 'function') {
                window.userTracker.updateSessionActivity();
            }
            
            // 세션 번호 증가 (한 번만)
            if (!window.sessionIncremented && window.sessionNumber) {
                window.sessionNumber++;
                window.sessionIncremented = true;
            }
        } catch (e) {
            console.warn('🔧 전역 세션 활동 업데이트 오류:', e);
        }
    };
}

// 전역 초기화 플래그들 (중복 방지)
if (window.thinkingDataSDKInitialized) {
    console.log('ℹ️ ThinkingData SDK가 이미 초기화됨');
} else {
    window.thinkingDataSDKInitialized = true;
    ...
    function setupAutoEventTracking() {
      // 중복 실행 방지
      if (window.autoEventTrackingInitialized) {
        console.log('ℹ️ 자동 이벤트 추적이 이미 초기화됨');
        return;
      }
      window.autoEventTrackingInitialized = true;
      ...
      try {
        ...
        // 2. 클릭 이벤트 자동 추적 (커스텀 처리로 변경) - 중복 방지
        if (!window.clickEventTrackingInitialized) {
          window.clickEventTrackingInitialized = true;
        ...
        // 3. 폼 제출 자동 추적 (커스텀 처리로 변경) - 중복 방지
        if (!window.formEventTrackingInitialized) {
          window.formEventTrackingInitialized = true;
        ...
        // 4. 외부 링크 클릭 자동 추적 (커스텀 처리로 변경) - 중복 방지
        if (!window.externalLinkTrackingInitialized) {
          window.externalLinkTrackingInitialized = true;
          ...
          console.log('✅ 외부 링크 클릭 자동 추적 설정 완료');
        }
    ...
    function initializeThinkingData() {
      // 중복 실행 방지 (5초 내 재실행 방지)
      const now = Date.now();
      if (window.thinkingDataState.sdkInitialized && (now - window.thinkingDataState.lastInitTime) < 5000) {
        console.log('ℹ️ ThinkingData SDK 초기화가 최근에 완료됨, 스킵');
        return;
      }
      ...
      try {
        ...
        // 상태 업데이트
        window.thinkingDataState.sdkInitialized = true;
        window.thinkingDataState.lastInitTime = now;
    ...
    function startAllTracking() {
      // 중복 실행 방지 (강화된 체크)
      if (window.thinkingDataState.trackingStarted) {
        console.log('ℹ️ ThinkingData 추적 시스템이 이미 시작됨');
        return;
      }
      
      // 5초 내 재실행 방지
      const now = Date.now();
      if (window.thinkingDataState.lastInitTime && (now - window.thinkingDataState.lastInitTime) < 5000) {
        console.log('ℹ️ ThinkingData 추적 시스템이 최근에 초기화됨, 스킵');
        return;
      }
      ...
      // 자동 이벤트 수집 설정 (SDK 기본 기능) - 중복 방지
      if (!window.autoEventTrackingInitialized) {
        setupAutoEventTracking();
        initializedCount++;
      }
      
      // 각 모듈의 초기화 함수 호출 (커스텀 모듈) - 중복 방지 강화
      ...
      // 상태 업데이트
      window.thinkingDataState.trackingStarted = true;
      window.thinkingDataState.lastInitTime = now;
      ...
      window.debugThinkingData = function() {
        ...
        console.log('- 초기화 상태:', window.thinkingDataState);
        console.log('- 중복 방지 플래그:', {
          autoEventTracking: window.autoEventTrackingInitialized,
          popupTracking: window.popupTrackingInitialized,
          clickTracking: window.clickTrackingInitialized,
          scrollTracking: window.scrollTrackingInitialized,
          formTracking: window.formTrackingInitialized,
          videoTracking: window.videoTrackingInitialized,
          resourceTracking: window.resourceTrackingInitialized,
          exitTracking: window.exitTrackingInitialized,
          userAttributeTracking: window.userAttributeTrackingInitialized
        });
      };
    }
